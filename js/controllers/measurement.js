angular.module('controllers').controller('measurementController', [
		'$scope', '$timeout', '$q', 'Measurement', '$routeParams', '$filter', 'Sensor',
		function ($scope, $timeout, $q, Measurement, $routeParams, $filter, Sensor) {
			...
			function onDataFetchSuccess() {
				...
				//measurements = $filter('orderBy')(measurements, 'date');
				var i=0;
			...
			function doQuery(number, typeOfMeas) {
				var d = $q.defer();
				var result = Measurement.query({id: number, type: typeOfMeas}, function (data) {
				    d.resolve(result);
				});
